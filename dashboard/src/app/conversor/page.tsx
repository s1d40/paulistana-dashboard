...
export default function App() {
  ...
  const fetchJobs = async () => {
    try {
      const response = await fetch("/api/conversor-backend/jobs");
      if (response.ok) {
  ...
  const handleStartConversion = () => {
    ...
    xhr.onload = async () => {
      if (xhr.status === 200) {
        try {
          ...
          // Trigger conversion
          const convertRes = await fetch("/api/conversor-backend/convert", {
            method: "POST",
            headers: {
    ...
    xhr.open("POST", "/api/conversor-backend/upload", true);
    xhr.send(formData);
  };

  const handleDownloadJob = (jobId: string) => {
    const link = document.createElement("a");
    link.href = `/api/conversor-backend/download/${jobId}`;
  ...
  const handleDeleteJob = async (jobId: string) => {
    try {
      const response = await fetch(`/api/conversor-backend/job/${jobId}`, {
        method: "DELETE",
      });
      if (response.ok) {
        fetchJobs();
